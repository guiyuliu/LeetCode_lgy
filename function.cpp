...
/* problem 2求字符串最长无重复子串的长度 。思路：两个变量，起点固定，终点往后挪。把这个子串中的每一个加到trace中，
 * 对trace使用find函数，找有无重复的。如果没有，把当前字符加入trace，终点往后挪；如果有，起点往后挪strat=found+1，
 * 然后终点再往后挪
 * 同时更新最长字符的长度ret。
 *
输入：string
